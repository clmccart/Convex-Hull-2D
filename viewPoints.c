...
/* initialize the array of points stored in global variable points[]
   with random points in two columns */
void initialize_points_columns() {
  //points must be allocated 
  assert(points);
  // seed randomness
  srand(time(NULL));
  ...
  for (int i=0; i < n; i++) {
    points[i].x = (int)(((i % 2) + 1) * WINDOWSIZE / 3.0);
    points[i].y = (int)(.2*WINDOWSIZE) + rand() % ((int)(.6*WINDOWSIZE));
  }
}
